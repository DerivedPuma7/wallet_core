...
func (c *ClientDb) Get(id string) (*entity.Client, error) {
	...
	stmt, err := c.DB.Prepare("SELECT id, name, email, createdAt, updatedAt FROM clients WHERE id = ?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
...
func (c *ClientDb) Save(client *entity.Client) error {
	stmt, err := c.DB.Prepare("INSERT INTO clients (id, name, email, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)")
	if err != nil {
		return err
	}
	defer stmt.Close()
